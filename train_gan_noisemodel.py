# python train_gan_noisemodel.py --batch_size 1 --gpus 1 --noiselist shot_read_uniform_row1_rowt_fixed1_periodic_learnedfixed --dataset semidark --crop_size 526
...
def main():
    ...
    parser.add_argument('--dataset', default='color_gray', help = 'Choose which dataset to use. Options: gray, color, semidark')
...
def get_dataset(args):    
    ...
    if 'semidark' in args.dataset:
        # overwriting composed_transforms here
        composed_transforms = torchvision.transforms.Compose([dset.ToTensor2(), dset.AddFixedNoise(), dset.RandCrop_gen_lowlightcams(shape = (args.crop_size,args.crop_size))])

        print('NOTE', 'Using 18 for training and 4 for testing')
        all_files_mat = glob.glob('data/paired_data/starlight-clean-noisy-pairs/*.mat')[0:18]
        all_files_mat_test = glob.glob('data/paired_data/starlight-clean-noisy-pairs/*.mat')[18:]
    
        dataset_train_real = dset.Get_sample_batch_semidark(all_files_mat, composed_transforms)
        dataset_test_real = dset.Get_sample_batch_semidark(all_files_mat_test, composed_transforms2)
...
def train(gpu, args):
    print('entering training function, gpu', gpu)
